...
interface CreatePasswordStepProps {
  ...
  goToStep: (step: 'welcome' | 'verify' | 'personal' | 'password' | 'forgot-password' | 'login' | 'dashboard') => void;
}

export function CreatePasswordStep({ authData, updateAuthData, goToStep }: CreatePasswordStepProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = { step: 4, email: authData.email, password };
      const response = await axiosInstance.post(AK.REGISTER, payload);
      console.log("response", response.data.data.user?.name)
      if (response.status >= 200 && response.status < 300) {
        const user = response.data.data.user;  // ✅ full user object
        const token = response.data.data.token;

        updateAuthData({ password });
        setUserData(user, token);  // ✅ store full object
        navigate('/dashboard');
        ...
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              >
                <input
                  ...
                  className={`flex h-10 px-3 py-2 pr-10 items-center gap-1.5 w-full rounded-lg border bg-white text-[#272727] text-sm font-normal leading-[100%] placeholder:text-[#272727] focus:outline-none focus:ring-2 focus:ring-[#124cbe] focus:border-transparent ${error ? "border-red-500" : "border-[#868EA4]"
                    }`}
            ...
            <button
              ...
              className={`flex h-9 px-5 py-2.5 justify-center items-center gap-1 w-full bg-[#124CBE] hover:bg-[#002f8b] disabled:bg-gray-400 disabled:cursor-not-allowed text-white text-center text-sm font-medium leading-[100%] rounded-lg transition-colors ${loading ? "opacity-50 cursor-not-allowed" : ""
                }`}
            ...
            >
              {loading ? (
                <Bars
                  ...
                  width="30"
                  color="#fff"
                  ariaLabel="bars-circles-loading"
                  wrapperStyle={{}}
                  wrapperClass=""
                />
              ) : (
