...
const PaymentsTable: React.FC = () => {
  ...
  const PaymentDataFetch = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await axiosInstance.post(AK.PAYMENTS); // Adjust to GET if needed
      const { success, data } = response.data;

      if (success && data.payments) {
        // Assuming data.payments is an array matching the Payments interface
          ...
          <div className="flex flex-col md:flex-row justify-between p-4 items-center">
            <div className="flex bg-[#EBEBEB] rounded-[4px] border border-[#EBEBEB] items-center gap-2 text-[#272727]  text-[16px] font-semibold">
              ...
              >
                ...
                >
                  {paymentsData.filter((item) => parseDate(item.dateTime) <= currentDate).length}
                </div>
              </button>
              <button
                className={`flex items-center gap-2 h-[36px] px-[16px] py-[8px] ${
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map((item, index) => (
                    <tr key={item.id} className="border-b border-b-[#F4F5F7]">
                      <td className="px-4 py-2 font-[500]">{(currentPage - 1) * itemsPerPage + index + 1}</td>
                      ...
                      <td className="px-4 py-2 font-[500]">
                        ...
                        >
                          {item.status === 'Success' ? <SvgIcons.TableSuccess /> : <SvgIcons.TableFail />}
                          {item.status}
                        </span>
                      </td>
