...
const ReportsTable: React.FC = () => {
  ...
  const fetchReportsData = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await axiosInstance.post(AK.REPORTS); // Using AK.REPORTS
      const { success, data } = response.data;

      if (success && data.reports) {
        // Assuming data.reports is an array matching the Reports interface
          ...
          <div className="flex flex-col md:flex-row justify-between p-4 items-center">
            <div className="flex bg-[#EBEBEB] rounded-[4px] border border-[#EBEBEB] items-center gap-2 text-[#272727]  text-[16px] font-semibold">
              ...
              >
                ...
                >
                  {reportsData.filter((item) => parseDate(item.dateTime) <= currentDate).length}
                </div>
              </button>
              <button
                className={`flex items-center gap-2 h-[36px] px-[16px] py-[8px] ${
