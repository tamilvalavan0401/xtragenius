...
const PayoutTable: React.FC = () => {
  ...
  const fetchPayoutData = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await axiosInstance.post(AK.PAYOUTS); // Using AK.PAYOUTS
      const { success, data } = response.data;

      if (success && data.payouts) {
        // Assuming data.payouts is an array matching the Payout interface
  ...
  let filteredData = payoutsData.filter((item) => {
          ...
          <div className="flex flex-col md:flex-row justify-between p-4 items-center">
            <div className="flex bg-[#EBEBEB] rounded-[4px] border border-[#EBEBEB] items-center gap-2 text-[#272727]  text-[16px] font-semibold">
              ...
              >
                ...
                >
                  {payoutsData.filter((item) => parseDate(item.dateTime) <= currentDate).length}
                </div>
              </button>
              <button
                className={`flex items-center gap-2 h-[36px] px-[16px] py-[8px] ${
